...
--Bajarla Reflection
In this activity, we explored the differences between sequential and parallel algorithms by implementing sorting and searching tasks. We observed that sequential algorithms follow a single, linear control flow, while parallel algorithms divide problems into subproblems to execute simultaneously across multiple processes.

Through testing with small, medium, and large datasets, we found that parallel algorithms are not inherently faster. For small workloads of 1,000 elements, sequential algorithms often outperformed parallel ones because the coordination costs and process creation overhead outweighed the computational benefits.
